...
#+begin_src conf
  bar {
      status_command i3status --config ~/.config/i3status/config-${HOSTNAME}
      position bottom
      workspace_buttons yes
      font pango:JetBrainsMonoNL Nerd Font Mono 8
      mode hide
      colors {
          statusline          $color06_i3wm
          background          $background_i3wm
          separator           $color03_i3wm
          #                   BORDER          BACKGROUND        TEXT
          focused_workspace   $color00_i3wm   $color03_i3wm     $color06_i3wm
          inactive_workspace  $color00_i3wm   $color00_i3wm     $color06_i3wm
          active_workspace    $color00_i3wm   $color02_i3wm     $color06_i3wm
          urgent_workspace    $color01_i3wm   $color01_i3wm     $color11_i3wm
          binding_mode        $color03_i3wm   $color01_i3wm     $color10_i3wm
          }
  }
#+end_src
...
#+begin_src conf
  ...
  exec --no-startup-id /usr/bin/xset s 360 360 # set blank time
  exec --no-startup-id /usr/bin/xset dpms 600 0 0 # set standby 600s, 0 suspend, and 0 off
#+end_src
...
#+begin_src conf
  ...
  bindsym $mod+Print exec ~/.local/bin/i3-screencast
#+end_src
...
#+begin_src conf
  ...
  bindsym $mod+F11 exec playerctl -p mpd play-pause
  bindsym $mod+F12 exec playerctl -p mpd next
  bindsym $mod+F10 exec playerctl -p mpd previous
...
*** mouse cursor zoom
#+begin_src conf
  bindsym --whole-window $mod+button4 exec boomer
  bindsym --whole-window $mod+button5 exec boomer
#+end_src
This will need the package [[https://github.com/tsoding/boomer][boomer]].
* i3status
** the default
:PROPERTIES:
:header-args: :tangle ./.config/i3status/config :mkdirp t
:END:
I don't mess much with /i3status/, it's usually hidden most of the times. Also since I'm already use /conky/ there's not much else to show in /i3status/. This should be used if there's no ~~/.config/i3status/config-${HOSTNAME}~.
*** header                                                         :noexport:
#+begin_src conf
...
#+begin_src conf
  ...
  format_degraded = "<span foreground='#eceff4'>󰍛 < %available </span>"
  }
#+end_src
*** time
#+begin_src conf
...
** kenya
:PROPERTIES:
:header-args: :tangle ./.config/i3status/config-kenya :mkdirp t
:END:
I don't mess much with /i3status/, it's usually hidden most of the times. Also since I'm already use /conky/ there's not much else to show in /i3status/. This should be used if there's no ~~/.config/i3status/config-${HOSTNAME}~.
*** header                                                         :noexport:
#+begin_src conf
...
#+begin_src sh
  ...
  # Run xidlehook
  xidlehook --socket /var/tmp/xidlehook.socket \
  	--detect-sleep \
      ...
      --timer 120 \
      'i3lock-fancy -f Overpass-Nerd-Font-Reg -d ' \
  	''
#+end_src
...
** i3-screencast
:PROPERTIES:
:header-args: :tangle ./.local/bin/i3-screencast :mkdirp t :shebang #!/usr/bin/env bash
:END:
...
#+begin_src sh
  ...
  if [ -s "${PIDFILE}" ] && [ -d "/proc/$(cat ${PIDFILE})" ]; then
  	...
  	notify-send -a "i3-screen-record" -t 3000 "saved to ${FINALFILE}"
