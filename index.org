...
- [[https://archlinux.org/packages/?name=i3-wm][i3-wm]] :: /Archlinux/ package for i3-wm. Also install the optional dependencies:
  - [[https://wiki.archlinux.org/title/Dmenu][dmenu]] :: a fast and lightweight dynamic menu for X.
...
** Optional packages
- archlinux-wallpaper :: for initial wallpapers.
...
:END:
:LOGBOOK:
- State "TODO"       from              [2025-01-01 Wed 18:23]
:END:
/archlinux/ users would want to install [[https://archlinux.org/packages/?name=xorg-xinit][xorg-xinit]] first.
...
:LOGBOOK:
- State "TODO"       from              [2025-01-01 Wed 18:24]
:END:
#+begin_src sh
  # ~/.xinitrc
...
:LOGBOOK:
- State "TODO"       from              [2025-01-01 Wed 18:25]
:END:
I sometimes used [[https://github.com/plexus/chemacs2][chemacs2]] for using multiple /Emacs/ configuration at once.
#+begin_src sh
...
**** cursor
:PROPERTIES:
:header-args: :tangle .config/Xresources.d/cursor
:END:
...
#+begin_src sh -n
  ...
  systemctl --user import-environment DISPLAY XDG_CURRENT_DESKTOP QT_QPA_PLATFORMTHEME QT_XCB_GL_INTEGRATION
#+end_src

**** 01-bluetooth.sh
:PROPERTIES:
:header-args: :tangle .config/xprofile.d/01-bluetooth.sh :shebang #!/bin/sh
:END:
...
**** 02-cursor.sh
:PROPERTIES:
:header-args: :tangle .config/xprofile.d/02-cursor.sh :shebang #!/bin/sh
:END:
#+begin_src sh
...
**** 03-udiskie.sh
:PROPERTIES:
:header-args: :tangle .config/xprofile.d/03-udiskie.sh :shebang #!/bin/sh
:END:
...
#+begin_src conf
  ...
  exec --no-startup-id dbus-launch dunst -conf ~/.config/dunst/dunstrc &
  ...
  #exec --no-startup-id feh --no-fehbg --bg-fill '/home/mkncorp.com/kristian.alexander/.local/share/wallpapers/manga/Sora-no-Otoshimono/angels_sora_no_otoshimono_ikaros_anime_anime_g_by_kardolen65_d6wp5ec.jpg'
...
#+begin_src sh
  ...
  xidlehook \
    ...
    --timer 10 \
      'xrandr --output "$PRIMARY_DISPLAY" --brightness 1; i3lock-fancy -d ' \
      '' \
